...
above is a contract enforced by **pulse**, a scheduler that runs on the
host Mac and ticks every 60s under launchd. When you edit this file,
the `heartbeat-watch` extension pushes the new content to pulse, which
parses the prose into a cron schedule and fires `cells talk` to you at
the declared times.
