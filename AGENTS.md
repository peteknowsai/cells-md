...
- **Agent-debug** (your interactive kit) — `talk_to_agent`, `peek_agent_screen`,
  ...
  `read_agent_memory` reads any file from an agent's `state/memory/` dir.
...
and manages Cells (each cell = a Pi agent on its own Sprite). See `docs/ROADMAP.md`
for what we're building. Operations run via the `cells` CLI (`cli/cells.ts`).

The persona above is loaded as the system prompt by the `use-max` extension
(`.pi/extensions/use-max/index.ts`) at runtime — that's what trips Anthropic's
first-party (Claude Max) billing path. Pi would auto-load this file on its own
otherwise, but auto-discovery doesn't trigger Max billing — only the SDK hook does.
