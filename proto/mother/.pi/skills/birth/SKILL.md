...
`cell-base` already has bun, pi-coding-agent, terminal toolkit, the DNA at `/cell` (with `__NAME__` / `__MODEL__` / `__PROVIDER__` / `__THINKING__` / `__MODEL_CHAIN__` placeholders intact), `bun install` done, pi-ai patches applied, and `/etc/profile.d/cells-env.sh` re-exporting `CELLS_PROXY_SECRET` under the names pi-ai expects (`ANTHROPIC_OAUTH_TOKEN`, `ANTHROPIC_AUTH_TOKEN`, `OPENAI_CODEX_API_KEY`). Birth's job is the per-cell delta: identity substitution, tmux color, optional extensions, site service.
...
Take that key-value list and pass it via `well_create`'s `env` parameter — welld lands each pair in `/etc/environment` on the well at first boot, PAM auto-loads it on every shell, and the in-image `/etc/profile.d/cells-env.sh` shim re-exports under the names pi-ai expects.
...
🚨 **The `env: [...]` parameter is NOT optional.** If you skip it, `/etc/environment` will be empty of secrets, the in-image `/etc/profile.d/cells-env.sh` shim will silently no-op (it's conditional on `CELLS_PROXY_SECRET` being set), and the cell will fail step 4b verify. Every birth that has dropped this parameter has failed in exactly the same place. Pass the env every time, even if the secrets list is short.
...
The DNA in `/cell` is intact from the bake but still has placeholders. Use `well_exec` to substitute them:
...
sed -i 's/__THINKING__/<THINKING>/g' /cell/.pi/settings.json
...
sed -i 's|__MODEL_CHAIN__|<CHAIN_JSON>|g' /cell/.pi/settings.json
```
...
The DNA's `/cell/.tmux.conf` ships with `__CELL_FG__` / `__CELL_BG__` / `__NAME__` placeholders in the status-left chip. Compute the color locally and substitute on the well:
...
sed -i "s|__CELL_BG__|$CBG|g; s|__CELL_FG__|$CFG|g; s|__NAME__|<NAME>|g" /cell/.tmux.conf
```
...
The right side of the tmux bar reads `/cell/.pi/status.json`. Write it now via `well_exec`:

```bash
mkdir -p /cell/.pi
cat > /cell/.pi/status.json <<'EOF'
{
  "harness": "<HARNESS>",
  "channels": []
}
...
The cell-base image ships with all four optional in-tree extensions (`memory`, `mentality`, `wiki`, `dream`) under `/cell/.pi/extensions/`. Pi only loads what's listed in `.pi/settings.json` `extensions` — leaving an extension on disk without registering it does nothing.
...
jq --arg p ".pi/extensions/<name>/index.ts" \
  '.extensions += [$p]' /cell/.pi/settings.json \
  > /tmp/s.json && mv /tmp/s.json /cell/.pi/settings.json
```
...
The cell's public face at `<NAME>.cells.md` is served by the cell itself — `/cell/site/server.ts` is a Bun web server that the cell owns. The site server also spawns `pi --mode rpc` as a child and exposes `/agent` over WebSocket. The per-cell Cloudflare Worker (deployed by the `cells` CLI post-birth, not from inside this skill) holds a persistent outbound WebSocket to that endpoint.
...
```bash
cd /cell && source /etc/profile.d/cells-env.sh && timeout 30 pi --print "say ok"
```

The profile.d sourcing matters: `well_exec` shells out via `bash -c` (not `-lc`), which skips `/etc/profile` and therefore skips `/etc/profile.d/cells-env.sh` — the shim that re-exports `CELLS_PROXY_SECRET` from `/etc/environment` as `ANTHROPIC_OAUTH_TOKEN`/`ANTHROPIC_AUTH_TOKEN`/`OPENAI_CODEX_API_KEY` (the names pi-ai's auth dispatch expects). Sourcing it explicitly mirrors what the site service does in `register-site-service.sh` — so a passing verify here proves the same env that production pi will have.
