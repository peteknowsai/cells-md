...
- **1 foreman cell**: `foreman` — also a normal cell, but additionally carries the `deliberate` extension (in `extension/deliberate/`). Foreman talks to the jury via Sprites HTTP API exec → `pi -p` on each juror in parallel.
