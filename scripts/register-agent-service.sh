...
#   3. Start tmux detached (`-d`) with a session named after the cell,
#      running Pi. `-A` makes it idempotent (attach if exists, create
#      otherwise).
...
SCRIPT="cd /home/sprite/agent && for f in /home/sprite/.bashrc.d/*; do . \$f; done; export PATH=/home/sprite/.local/bin:\$HOME/.bun/bin:\$PATH; tmux new-session -dA -s $NAME pi && while tmux has-session -t $NAME 2>/dev/null; do sleep 10; done"
