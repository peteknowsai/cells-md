...
- [ ] **V1.STEP3** Hibernated cell-base pool (Tier 3 — pi pre-running). Bake a special "egg" cell variant: forked from cell-base, booted, pi started + sitting at idle, then hibernated via wells's `well stop --hibernate` (or equivalent). Pool keeps 1 warm hibernated egg. Birth grabs egg, `well wake` (~3s), egg already has pi alive → drop into talk. Refill triggers async after consumption. Falls back to cold-fork if pool empty. **Where the real speed comes from** — should drop birth from 10s → ~3s. Owner: `worker`. Depends: V1.STEP2.
