...
_(empty — Phase v1 planned, ready to start)_
...
- [ ] **V1.STEP1** Bake canned generic cell identity into cell-base. Write a generic `IDENTITY.md` (cell voice, no harness reference, no model reference). Set default `status.json` with placeholder name. Set default `settings.json` with model chain + always-on extensions enabled. Update bake recipe in `cli/cells.ts` cmdBake. Owner: `worker`.
- [ ] **V1.STEP2** Refactor `cmdCreate` fast-path: deterministic TS function, no mother in critical path. Background birth promise + foreground Ink animation + await on first-user-send. Auto-name cells (`cell-<short-id>` or similar). Mother's slow-birth stays as fallback for non-v1 custom births. Owner: `worker`. Depends: V1.STEP1.
- [ ] **V1.STEP3** React Ink birth-animation component (`cli/birth-ui.tsx`). 4 stages × 750ms (waking → warming → ready → alive). Filling dots + label line beneath. Mounts on `cells birth`, unmounts at animation end, cleanly hands off to talk prompt. Owner: `worker`.
- [ ] **V1.STEP4** Hibernated cell-base pool. 1 hot egg always warm. `cells egg refill` keeps the pool at depth 1. Refill triggered after consumption. Birth grabs warm egg from pool; falls back to cold-fork if empty. Owner: `worker`. Depends: V1.STEP2.
- [ ] **V1.STEP5** (Optional) Well-rename support via welld API. Check if `well rename` or equivalent exists. If yes, egg from pool gets renamed to cell name at birth (friendlier UX). If not, skip and use egg's pool-name as cell-name for v1. Owner: `worker`. Depends: V1.STEP4.
- [ ] **V1.STEP6** Live test the full birth flow from cold start. Tune animation timing + first-LLM-token latency. Capture numbers in `docs/perf/birth-to-greeting.md`. Owner: `worker`. Depends: V1.STEP4 (V1.STEP5 optional).
