...
- **`cell`** (uid 1002, sudo group). The tenant user. Created by `cells bake`'s `bakeCreateCellUser`; HOME is `/root` (not `/home/root`). Pi runs as `cell`, the agent's DNA + memory + site + node_modules all live under `/root/`. SSH'ing into a cell with `cells shell <name>` targets the `cell` user. `well exec ... -- sudo -u cell bash -c '...'` is the canonical way to run as cell from the substrate side; `wellExecCapture(name, script, {user: "cell"})` and the mother's `well_exec` tool both default to user=cell.
...
Paths in remote command bodies should be absolute (`/root/...` for tenant content, `/home/well/...` only for substrate bookkeeping). Tilde expansion depends on which user runs the shell — prefer explicit paths.
